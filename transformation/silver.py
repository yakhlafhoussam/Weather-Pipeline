...
def clean_weather(df):
    ...
    df = df[(df["precipitation_sum"] >= 0) & (df["wind_speed_max"] >= 0)]
...
def run_silver(weather_path, output_path):

    print("Start Silver part...")

    weather_data = load_weather(weather_path)

    df = flatten_weather(weather_data)

    df = clean_weather(df)

    save_silver(df, output_path)

    print("Finish Silver part")
